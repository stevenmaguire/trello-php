    ...
    private static function _iteratorToArray($iterator)
    {
        ...
        for ($iterator->rewind(); $iterator->valid(); $iterator->next()) {
            ...
            if ($parentElement[0] instanceof SimpleXMLIterator) {
                ...
                $parentKey = Trello_Util::delimiterToCamelCase($parentElement->getName());
            } else {
                $parentElement = null;
            }
            ...
            if ($parentKey == "customFields") {
                $key = Trello_Util::delimiterToUnderscore($iterator->key());
            } else {
                $key = Trello_Util::delimiterToCamelCase($iterator->key());
            }
